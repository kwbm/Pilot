...
layout(input_attachment_index = 0, set = 1, binding = 0) uniform highp subpassInputMS in_gbuffer_a;
layout(input_attachment_index = 1, set = 1, binding = 1) uniform highp subpassInputMS in_gbuffer_b;
layout(input_attachment_index = 2, set = 1, binding = 2) uniform highp subpassInputMS in_gbuffer_c;
layout(input_attachment_index = 3, set = 1, binding = 3) uniform highp subpassInputMS in_scene_depth;
...
void main()
{
    highp vec3 final_color = vec3(0.0, 0.0, 0.0);

    for (int i = 0; i < msaa_sample_count; i++)
    {
        PGBufferData gbuffer;
        highp vec4   gbuffer_a = subpassLoad(in_gbuffer_a, i).rgba;
        highp vec4   gbuffer_b = subpassLoad(in_gbuffer_b, i).rgba;
        highp vec4   gbuffer_c = subpassLoad(in_gbuffer_c, i).rgba;
        ...
        {
            highp float scene_depth              = subpassLoad(in_scene_depth, i).r;
        ...
        if (SHADINGMODELID_UNLIT == gbuffer.shadingModelID)
        {
            ...
            final_color = textureLod(skybox_sampler, origin_sample_UVW, 0.0).rgb;
...
#include "mesh_lighting.inl"
            ...
            final_color += result_color / float(msaa_sample_count);
        }
    }
